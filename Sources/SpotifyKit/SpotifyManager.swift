...
public class SpotifyManager {
    ...
    @objc(SpotifyKit)private class SpotifyToken: NSObject, Decodable, NSCoding {
        ...
        func refresh(from data: Data) {
            guard let token = try? JSONDecoder().decode(SpotifyToken.self, from: data) else { return }
     ...
     */
    public func get<T>(_ what: T.Type, id: String, completionHandler: @escaping ((T) -> Void)) where T: SpotifySearchItem {
        tokenQuery { token in
            URLSession.shared.request(SpotifyQuery.urlFor(what, id: id), method: .get, headers: self.authorizationHeader(with: token))
            { result in
     ...
     */
    public func search<T>(_ what: T.Type, _ keyword: String, completionHandler: @escaping ([T]) -> Void) where T: SpotifySearchItem {
        tokenQuery { token in
            URLSession.shared.request(SpotifyQuery.search, method: .get, parameters: self.searchParameters(for: what.type, keyword), headers: self.authorizationHeader(with: token))
            { result in
                if case let .success(data) = result, let results = try? JSONDecoder().decode(SpotifySearchResponse<T>.self, from: data).results.items {
     ...
     */
    public func getTrack(title: String, artist: String, completionHandler: @escaping (SpotifyTrack) -> Void) {
        search(SpotifyTrack.self, "\(title) \(artist)") { results in
    ...
    public func myProfile(completionHandler: @escaping (SpotifyUser) -> Void) {
        tokenQuery { token in
            URLSession.shared.request(SpotifyQuery.me, method: .get, headers: self.authorizationHeader(with: token))
            { result in
                if  case let .success(data) = result, let result = try? JSONDecoder().decode(SpotifyUser.self, from: data) {
                    completionHandler(result)
                }
            }
        }
    ...
    public func saveToken(from authorizationCode: String) {
        ...
        URLSession.shared.request(SpotifyQuery.token, method: .post, parameters: tokenParameters(for: application, from: authorizationCode))
        { result in
            if case let .success(data) = result {
    ...
    public func saveToken(accessToken: String, expiresIn: Int, refreshToken: String, tokenType: String) {
        self.token = SpotifyToken(accessToken: accessToken, expiresIn: expiresIn, refreshToken: refreshToken, tokenType: tokenType)
    ...
    public func refreshToken(completionHandler: @escaping (Bool) -> ()) {
        ...
        URLSession.shared.request(SpotifyQuery.token, method: .post, parameters: refreshTokenParameters(from: token), headers: refreshTokenHeaders(for: application))
        { result in
            if case let .success(data) = result {
                ...
                if let token = self.token {
                    ...
                    // Run completion handler only after the token has been saved
     ...
     */
    public func library<T>(_ what: T.Type, completionHandler: @escaping ([T]) -> Void) where T: SpotifyLibraryItem {
        tokenQuery { token in
            URLSession.shared.request(SpotifyQuery.libraryUrlFor(what), method: .get, headers: self.authorizationHeader(with: token))
            { result in
                if case let .success(data) = result, let results = try? JSONDecoder().decode(SpotifyLibraryResponse<T>.self, from: data).items {
                    completionHandler(results)
                }
            }
        }
    ...
    public func save(trackId: String, completionHandler: @escaping (Bool) -> Void) {
        tokenQuery { token in
            URLSession.shared.request(SpotifyQuery.libraryUrlFor(SpotifyTrack.self), method: .put, parameters: self.trackIdsParameters(for: trackId), headers: self.authorizationHeader(with: token))
            { result in
    ...
    public func delete(trackId: String, completionHandler: @escaping (Bool) -> Void) {
        tokenQuery { token in
            URLSession.shared.request(SpotifyQuery.libraryUrlFor(SpotifyTrack.self), method: .delete, parameters: self.trackIdsParameters(for: trackId), headers: self.authorizationHeader(with: token))
            { result in
                if case .success(_) = result {
                    completionHandler(true)
                } else {
    ...
    public func isSaved(trackId: String, completionHandler: @escaping (Bool) -> Void) {
        tokenQuery { token in
            URLSession.shared.request(SpotifyQuery.contains, method: .get, parameters: self.trackIdsParameters(for: trackId), headers: self.authorizationHeader(with: token))
            { result in
     ...
     */
    private func searchParameters(for type: SpotifyItemType, _ keyword: String) -> HTTPRequestParameters {
    ...
    private func refreshTokenHeaders(for application: SpotifyDeveloperApplication) -> HTTPRequestHeaders {
        guard let auth = URLSession.authorizationHeader(user: application.clientId, password: application.clientSecret) else {
            return [:]
        }
